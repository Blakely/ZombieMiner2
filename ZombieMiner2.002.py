...
#      - more on handleZombie to use dirtyPos instead of getPos, che
...
def scrollMap(screen,player,tilemap,moveVect):
    ...
    if (player.getPos()[X]<PLAYER_CENTERPOS[X] or player.getPos()[X]>tilemap.getSize()[X]-PLAYER_CENTERPOS[X] or player.getPos()[X]==PLAYER_CENTERPOS[X] and player.dir==DIR_RIGHT):
    ...
    if (player.getPos()[Y]<PLAYER_CENTERPOS[Y] or player.getPos()[X]>tilemap.getSize()[Y]-PLAYER_CENTERPOS[Y] or player.getPos()[Y]==PLAYER_CENTERPOS[Y] and player.dir==DIR_DOWN):
