    ...
    while play:
        ...
        if (not endWin):
            ...
            if(updateUI==WIN_STAT):
                statWin = createStatWin(player)
            if (updateUI==WIN_END):
                time=getTime(pygame.time.get_ticks() - startTime) #get time elapsed in (minutes,seconds) since the game started
                endWin=createEndWin("Got the Meth!","Time : " + time[0] + " minutes, " + time[1] + " seconds")
